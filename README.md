...
| Skill | 作用 |
|---|---|
| [stock-analysis](stock-analysis/) | 结合工作区账户/持仓/笔记，全面分析一只 A 股/ETF 是否值得买（已持仓时判断是否值得加仓）：证据先行、结论最后（基本面/技术面/支撑压力/风险/仓位匹配），并生成近期支撑位/压力位、买点、止损、止盈锚点与盈亏比参考 |
...
| [position-management](position-management/) | 持仓生命周期管理：建仓执行、加仓决策、清仓执行、平仓复盘与总结（生成 TRADE-SUMMARY.md 并归档） |
