...
| [limit-lifecycle](limit-lifecycle/) | 题材生命周期 | 读 $limit-find 的当日报告（没有当日报告先执行 $limit-find），给今日主线板块（含上一份本技能报告里仍在跟踪的板块）标注题材生命周期四阶段——点火/扩散/加速/退潮，每条阶段判断都跟数字理由与本轮第几天；再用主线健康度三检查点（龙头还在不在、涨停家数是增是减、资金是进还是退）判定这条线还活着、在收缩还是已退潮——每格按 正/弱/负/未知 记，正负都必须指向两个时点的可比数字，未知不计入正负（单日的资金只是状态，方向要今日 vs 基准日）；只有三格全正才算还活着，且推荐只限扩散阶段（点火只列观察、连续跟踪 3 天，加速与退潮不参与），两正一弱写「继续跟踪但不作为主线」；最后总结推荐的题材板块与板块内候选股票（角色/数字理由/观察条件，按 ACCOUNT.md 板块权限过滤无法买入的股票），写入 report/题材生命周期/YYYY-MM-DD.md；只做阶段判定与候选筛选，不给买点止损仓位、不下单不预测 |
...
limit-lifecycle 是可选的“题材生命周期”入口（不属于每日闭环）：先拿到 limit-find 的当日报告（没有当日报告就先执行 limit-find），把「今天钱在哪」升级成「这条线现在第几天」——给今日主线板块与上一份报告里仍在跟踪的板块标注点火/扩散/加速/退潮阶段与数字依据（含本轮第几天），跑主线健康度三检查点（龙头、涨停家数、资金），最后给出推荐的题材板块与板块内候选股票（角色、数字理由、观察条件）；报告写入 report/题材生命周期/YYYY-MM-DD.md，只做阶段判定与候选筛选，买卖判断仍走 stock-analysis → position-management。
