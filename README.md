...
| [limit-lifecycle](limit-lifecycle/) | 题材生命周期 | 读 $find-limit 的当日报告（没有当日报告先执行 $find-limit），给今日主线板块（含上一份本技能报告里仍在跟踪的板块）标注题材生命周期四阶段——点火/扩散/加速/退潮，每条阶段判断都跟数字理由与本轮第几天；再用主线健康度三检查点（龙头还在不在、涨停家数是增是减、资金是进是出）判定这条线还活着、在收缩还是已退潮；最后总结推荐的题材板块与板块内候选股票（角色/数字理由/观察条件，按 ACCOUNT.md 板块权限过滤无法买入的股票），写入 report/题材生命周期/YYYY-MM-DD.md；只做阶段判定与候选筛选，不给买点止损仓位、不下单不预测 |
...
```
使用 $skill-installer 从 GitHub 仓库 ddd-online/stock-skills 安装以下 skills：market-data、stock-analysis、find-leader、find-limit、limit-lifecycle、judge-limit、find-simmer、stock-report、position-management、setup-stock-workspace
```
...
find-limit 是可选的“涨停板数据查找”入口（不属于每日闭环）：用涨停/跌停/炸板家数、连板高度、晋级率与炸板率描述当日情绪与主线，并在最后一步把当日加之前四次报告共五份汇总出阵型四态；报告写入 report/涨停板复盘/YYYY-MM-DD.md，只做数据查找与判定，买卖判断仍走 stock-analysis → position-management。

limit-lifecycle 是可选的“题材生命周期”入口（不属于每日闭环）：先拿到 find-limit 的当日报告（没有当日报告就先执行 find-limit），把「今天钱在哪」升级成「这条线现在第几天」——给今日主线板块与上一份报告里仍在跟踪的板块标注点火/扩散/加速/退潮阶段与数字依据（含本轮第几天），跑主线健康度三检查点（龙头、涨停家数、资金），最后给出推荐的题材板块与板块内候选股票（角色、数字理由、观察条件）；报告写入 report/题材生命周期/YYYY-MM-DD.md，只做阶段判定与候选筛选，买卖判断仍走 stock-analysis → position-management。

judge-limit 是可选的“打板竞价评估”入口（不属于每日闭环）：9:25–9:30 读 $find-limit 的昨日涨停板复盘报告，输出昨日涨停股集体竞价表与板块竞价表（阵型沿用报告里的判定，本入口不重复判定），并按两条硬规则给结论——竞价量占比按「竞价成交额 ÷ 昨日全天成交额」计算；最高板今天天地板或跌停则当天只观察。没有昨日报告先跑 $find-limit，本入口只出观察结论，买卖仍走 stock-analysis → position-management。
...
调用约束：
- 前置：find-leader、find-limit、judge-limit 为纯数据技能，不读取工作区文件、可在任意目录运行（judge-limit 读取 report/涨停板复盘/ 下 $find-limit 的上一份报告作为样本来源，没有报告就说明原因结束）；limit-lifecycle 读取 report/涨停板复盘/ 下 $find-limit 的当日报告（没有当日报告先执行 $find-limit），并仅读取 ACCOUNT.md 的「板块权限」用于排除无法买入的股票；find-simmer 仅读取 ACCOUNT.md 的「板块权限」用于排除无法买入的股票；其余 SKILL 依赖工作区文件（ACCOUNT.md / NOTES.md / POSITION.md / MUST.md / stocks/），未初始化先运行 $setup-stock-workspace
- find-leader、find-limit、judge-limit 不读取 MUST.md；limit-lifecycle 与 find-simmer 仅按 ACCOUNT.md 板块权限过滤、不读取 MUST.md；其余 SKILL 必须遵守工作区 MUST.md 中的个人交易风格与规则（默认只有一个标题，由用户编辑）
