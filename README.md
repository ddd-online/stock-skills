...
中文 A 股实盘交易辅助的 Codex skills 集合：核心闭环覆盖「个股分析 → 交易计划 → 建仓/持仓 → 清仓复盘 → 统计归档」，板块龙头查找（leader-find）、涨停板查找（limit-find）、题材生命周期（limit-lifecycle）、打板竞价评估（limit-judge）、蓄力票发现（simmer-find）是五个可选查找与评估入口。每个 skill 独立自包含（SKILL.md + references + scripts），真实数据统一由 market-data 拉取（腾讯行情 + 东方财富行情/板块/财报/新闻/资金流/数据中心 F10 + 新浪行情与资金流公开接口）并输出数据报告——东财行情中心接口不可用时脚本按 auto 自动切腾讯/新浪/东财 F10 备用源并标注数据源与口径，无需密钥。

当前版本：7.6.0 · [查看发布记录](https://github.com/ddd-online/stock-skills/releases)

## Skills 一览

| Skill | 定位 | 作用 |
|---|---|---|
| [market-data](market-data/) | 数据层 | 统一拉取 A 股真实行情、财报、新闻公告、资金流向、强势股榜单、板块行情榜/成分股榜/板块财务排名、涨停板数据并输出数据报告，供其他 skill 调用；东财行情中心接口不可用时（板块行情榜/成分股榜/板块财务排名/强势股榜/资金流向）自动切腾讯行情、新浪行情与资金流、东财数据中心 F10 备用源，并标注数据源与口径 |
...
| [leader-find](leader-find/) | 板块龙头查找 | **必传一个板块参数（板块名或 BK 代码，没给就不运行、先向用户索取，也不做全市场扫描）**：先判这条线的题材生命周期阶段（点火/扩散/加速/收缩，当日单日口径），再按阶段认定有没有龙头（点火档全是首板＝一般无龙头、只标「先动股」；扩散档出现 2–3 板晋级股＝龙头候选；加速档龙头领涨且板块跟涨＝龙头已确认；收缩档写龙头断板后的四种走法），龙头候选按板块内五字段纵向比较排序（首封时间 → 连板高度 → 换手与量能 → 资金 → 炸板后的回封速度），跟风板/尾盘偷袭板/高位高换手板/独涨板降级为伪龙头；再做人气龙头四查打分（相对强度/硬逻辑占主营/量能健康/上车点）与行业龙头（营收净利第一梯队 + 质地 + 估值 + 主营验证）核验，最后把「最强」与「值得买」分开，写入 report/龙头扫描/YYYY-MM-DD-<板块名>.md；只出名单与观察条件，不下单、不给仓位 |
...
```
使用 $leader-find 分析半导体板块的龙头，把最强和值得买分开
...
交易线含义：setup-stock-workspace 只执行一次；之后每个交易日从“入口”进入分析线——入口有四个：用户直接要求分析个股、$leader-find 板块龙头查找、$limit-find 涨停板数据查找、$simmer-find 蓄力票发现；stock-analysis 的分析模式只分析并输出报告（不落盘、不维护观察池），“买入结论”经 position-management 落到股票持仓；已持仓由 stock-analysis 的持仓检查模式每日检查，stock-report 把检查结果汇总为复盘报告与明日预案，形成“当日复盘 → 次日 9:25 执行”的循环。
...
leader-find 是可选的“板块龙头查找”入口（不属于每日闭环）：**必须传入一个板块**（板块名或 BK 代码，没给就不运行、先向用户索取，也不做全市场扫描），然后经 $market-data 取该板块的板块行、全量成分股榜、板块财务排名与涨停池明细，先按题材生命周期阶段认定有没有龙头（点火＝无龙头、扩散＝龙头候选、加速＝龙头已确认、收缩＝断板后四走法），按五字段做板块内纵向排序并降级四种伪龙头，再叠加人气龙头四查与行业龙头（基本面第一梯队）核验，最后把「最强」与「值得买」分开；报告写入 report/龙头扫描/YYYY-MM-DD-<板块名>.md，只出名单与观察条件，买卖判断仍走 stock-analysis → position-management。
...
调用约束：
- 前置：leader-find、limit-find、limit-judge 为纯数据技能，不读取工作区文件、可在任意目录运行（leader-find 必须传入一个板块参数，未给板块时不运行、先向用户索取；limit-judge 读取 report/涨停板复盘/ 下 $limit-find 的上一份报告作为样本来源，没有报告就说明原因结束）；limit-lifecycle 读取 report/涨停板复盘/ 下 $limit-find 的当日报告（没有当日报告先执行 $limit-find），并仅读取 ACCOUNT.md 的「板块权限」用于排除无法买入的股票；simmer-find 先用 $market-data 板块行情榜取强势板块，仅读取 ACCOUNT.md 的「板块权限」用于排除无法买入的股票；其余 SKILL 依赖工作区文件（ACCOUNT.md / NOTES.md / POSITION.md / MUST.md / stocks/），未初始化先运行 $setup-stock-workspace
...
- leader-find 的板块参数必传且只识别不结论：没给板块名/BK 代码就不运行，也不做全市场扫描；只输出「这条线有没有龙头、谁是龙头/龙头候选」与「值得进一步评估」名单，不输出买卖结论；候选须经 $stock-analysis 输出建仓信号后才可走 $position-management
