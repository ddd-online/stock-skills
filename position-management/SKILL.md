...
1. 读取工作区上下文并收集输入：ACCOUNT.md（本金/现金/总盈亏）、POSITION.md（该股持仓与总仓位）、MUST.md（个人交易风格与必守规则）、股票代码、最大亏损预算（默认本金2%）、买点/止损/止盈/支撑压力与加仓建议（可来自 stock-analysis 的输出）。
2. 拉取当前价格确认买点条件仍成立：
...
3. 建仓：确认批次结构 → 计算股数与亏损 → 下单前检查 → 输出建仓计划卡。
...
- 仓位处理集中：所有仓位计算（批次、手数、费用、单笔亏损验证）由本 SKILL 负责；stock-analysis 只提供结论、锚点与加仓建议。
