...
- 交易费用设置（用于每次交易的费用计算，写入 ACCOUNT.md）：
  ...
  - 过户费：默认 0.001%（买卖双向；规则固定，可调整）
  - 过户费市场规则：确认深市是否已计入佣金（默认已计入、不单独收）、沪市是否单独收取（默认按 0.001% 单收；沪市 600/601/603/688 等）
  - 拿不到用户答案时用默认值，并在 ACCOUNT.md 标注“默认值，待确认”
- 板块权限（写入 ACCOUNT.md「板块权限」，供 find-simmer 等按权限排除无法买入的股票）：主板（默认已开通）、创业板、科创板、北交所是否开通，ST/风险警示股是否允许交易；拿不到答案时用默认（仅主板可交易、创业板/科创板/北交所未开通、ST 不交易）并标注“默认值，待确认”
...
- ACCOUNT.md：账户总览（本金、现金、总盈亏、累计支取、实际可用资产、资金变化记录）+ 交易费用设置（佣金费率/最低佣金/印花税/过户费与深/沪过户费口径）+ 板块权限（主板/创业板/科创板/北交所/ST，未开通的板块不交易）+ 资金规则（以实际可用资产 = 本金 + 总盈亏 − 累计支取 为基数：现金储备≥实际可用资产×30%（资产<5万免红线）、单笔预算≤实际可用资产×2%（连亏2笔降1%）、违规处置、当前档位）与资金变化记录备注口径（费用、净盈亏、违规标签），每次资金变动后按口径同步重算
