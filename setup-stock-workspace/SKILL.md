...
- ACCOUNT.md：账户总览（本金、现金、总盈亏、累计支取、实际可用资产、资金变化记录）+ 交易费用设置（佣金费率/最低佣金/印花税/过户费）+ 板块权限（主板/创业板/科创板/北交所/ST，未开通的板块不交易）+ 资金规则（以实际可用资产 = 本金 + 总盈亏 − 累计支取 为基数：现金储备≥实际可用资产×30%、单笔预算≤实际可用资产×2%、当前档位），每次资金变动后按口径同步重算
