...
| # | 路径 | 用途 |
|---|---|---|
| 1 | ACCOUNT.md | 账户总览（本金/现金/总盈亏/累计支取/实际可用资产/资金变化记录）+ 交易费用设置（佣金费率/最低佣金/印花税/过户费与深市已计入佣金、沪市单独收取的市场规则）+ 板块权限（主板/创业板/科创板/北交所/ST，未开通板块不交易）+ 资金规则（账户口径、现金储备≥实际可用资产30%（资产<5万免红线）、单笔预算≤实际可用资产2%（连亏2笔降1%）、违规处置、当前档位） |
...
| 5 | WATCHLIST.md | 观察池（标的/触发条件/预案/状态），由 stock-analysis 判定进出并更新 |
